...
const open = function(image, source, blur_image) {
    modal_image.classList.remove("loaded")
    modal_image_bg.classList.remove("loaded")
    modal_image.src = ''
    ...
    modal_image.style = blur_image + 'background-position: center; background-repeat: no-repeat; background-size: cover;';
    ...
    function loaded() {
        modal_image.classList.add("loaded")
...
img.forEach((item) => {
    let blur_image = item.parentElement.getAttribute("style");
    ...
    item.addEventListener("click", function() { open(image, source, blur_image) })
